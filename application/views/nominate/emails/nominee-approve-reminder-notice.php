	...
	<div class="container" style="background-color:#fff; padding-bottom:20px; padding-top:20px;">
        ...
        <div class="col-md-12">
			...
			<div class="" style="margin-bottom:10px; padding-left:10px; line-height:25px;">
				...
				Dear <strong> <?php if(!empty($managerDetail)){ echo ucfirst($managerDetail['line_manager_name']); }?> </strong>,<br>

				Please be informed that your Direct Report, <?php if(!empty($managerDetail)){ echo ucfirst($managerDetail['survey_participant_name']); }?>, 
				has been selected for a 180-degree Assessment.<br>
				<?php if(!empty($managerDetail)){ echo ucfirst($managerDetail['survey_participant_name']); }?> does not have the required number of Team Members 
				but has nominated the following non-team members to provide feedback on his/her leadership impact.<br><br>

				As <?php if(!empty($managerDetail)){ echo ucfirst($managerDetail['survey_participant_name']); }?> Line Manager, 
				we request that you kindly approve or reject <?php if(!empty($managerDetail)){ echo ucfirst($managerDetail['survey_participant_name']); }?> nominations. 
				...
				<a href="<?php echo base_url() .'nominate/approve/'. $managerDetail['survey_id'] .'/'. $managerDetail['survey_participant_id'] . '/'; ?>">
					<?php echo base_url() .'nominate/approve/'. $managerDetail['survey_id'] .'/'. $managerDetail['survey_participant_id'] . '/'; ?>
				</a> 		
				<br><br>
